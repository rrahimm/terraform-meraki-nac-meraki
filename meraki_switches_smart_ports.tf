    ...
    for domain in try(local.meraki.domains, []) : [
      for organization in try(domain.organizations, []) : [
        for profile in try(organization.switch_port_profiles, []) : {
          ...
          tags            = try(profile.tags, local.defaults.meraki.domains.organizations.switch_port_profiles.tags, null)
          # network_id is used for network-scoped profiles; resolved from network_name
          network_id = try(
          ...
          # networks_values resolves each network name to its ID
          ...
          # port_access_policy_number resolved from name via the network specified in port_access_policy_network
          port_access_policy_number = try(
            meraki_switch_access_policy.networks_switch_access_policies[
              format("%s/%s/%s/%s", domain.name, organization.name, profile.port_access_policy_network, profile.port_access_policy_name)
            ].id,
            null,
          )
          ...
          port_adaptive_policy_voice_group_id = try(
            local.organizations_adaptive_policy_group_ids[format("%s/%s/%s", domain.name, organization.name, profile.port_adaptive_policy_voice_group_name)],
            null,
          )
